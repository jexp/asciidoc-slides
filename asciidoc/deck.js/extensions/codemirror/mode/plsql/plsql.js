CodeMirror.defineMode("plsql", function(config, parserConfig) {
  ...
  var cKeywords = "abort accept access add all alter and any array arraylen as asc assert assign at attributes audit " +
        ...
        "join " +
        "key " +
        "left level library like limited local lock log logging long loop " +
  ...
  var indentUnit       = config.indentUnit,
      keywords         = splitKeywords(cKeywords),
      functions        = splitKeywords(cFunctions),
      types            = splitKeywords(cTypes),
      sqlplus          = splitKeywords(cSqlplus)
